<template>
  <div class="filter-content pb-2">
    <div class="overflow-hidden pt-1">
      <div @click="visible=!visible" class="inline-block float-right border rounded-sm px-1 mt-1 border-gray-400">
        <span class="iconfont icon-filter">筛选</span>
      </div>
    </div>
    <div class="block bg-gray-100 rounded-sm p-1 space-y-2 mt-2 py-2 overflow-hidden dark:bg-gray-800" v-if="visible">
      <div v-for="(filter, index) in rankFilterArray" :key="filter.paramsName">
        ...
        <div v-if="['multi', 'single'].includes(filter.type)" class="inline-block">
          ...
          >
            ...
            >
            <input v-else type="checkbox" class="hidden checkbox"
              :value="option.value" v-model="data[index]"
            >
            <div class="label">
              {{ option.text }}
        ...
        <div v-else>
          <div class="filter-input-area">
            <input type="text" v-model="data[index]" class="filter-input" placeholder="最多三个长度不超过10的关键词">
            <span class="iconfont icon-clean pl-1 w-3" @click="data[index] = ''"/>
          </div>
        </div>
      </div>
      <div class="button-panel pt-3 flex justify-center space-x-5 md:space-x-10 text-white">
